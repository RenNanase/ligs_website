...
import { KepuasanPelangganForm } from "@/components/kepuasan-pelanggan-form"
import { useState, useEffect } from "react"
import { AlertCircle } from "lucide-react"

export default function KepuasanPelangganPage() {
  const { t } = useLanguage()
  ...
  useEffect(() => {
    fetch("/api/survey-settings")
      .then((res) => res.json())
      .then((data) => setFormOpen(data.pelangganOpen ?? true))
  ...
  return (
    <>
      <PageHeader
        title={t("kepuasan.title")}
        subtitle={t("kepuasan.subtitle")}
      />
      <section className="bg-primary-bg py-16">
        <div className="mx-auto max-w-2xl px-6">
          <div className="mb-6 space-y-2 text-sm text-muted-foreground">
            <p>{t("kepuasan.intro1")}</p>
            <p>{t("kepuasan.intro2")}</p>
            <p>{t("kepuasan.intro3")}</p>
            <p>{t("kepuasan.intro4")}</p>
          </div>
